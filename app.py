...
def main() -> None:
    ...
    def update_timeline(
            click_data,
            selected_data):
        selected_items = _get_selected_items(click_data, selected_data)
        timeline = go.Figure(
            layout={
                'height': 360,
                'width': 640,
        ...
        timeline.add_trace(
            go.Bar(
                # dash does not seem to like timedelta, maybe related to issue 1808...
                x=(df['end'] - df['start']).astype('timedelta64[ms]').dt.days*24*60*60*1000,
                y=df['title'],
                orientation='h',
                base=df['start'],
    ...
    def update_globe(
            click_data,
            selected_data):
        scope = df.iloc[_get_selected_items(click_data, selected_data)]
